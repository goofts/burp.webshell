...
@PluginAnnotation(Name = JZip.CLASS_NAME, payloadName = "JavaDynamicPayload")
public class JZip implements Plugin {
    ...
    private void load() {
        if (!this.loadState) {
            try {
                InputStream inputStream = getClass().getResourceAsStream(String.format("assets/%s.classs", CLASS_NAME));
