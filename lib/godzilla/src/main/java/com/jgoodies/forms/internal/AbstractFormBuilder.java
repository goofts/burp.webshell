...
public abstract class AbstractFormBuilder<B extends AbstractFormBuilder<B>> extends AbstractBuilder<B> {
    ...
    public final void nextColumn(int columns) {
        cellConstraints().gridX += getColumnIncrementSign() * columns;
    }
    ...
    /* access modifiers changed from: protected */
    public final CellConstraints createLeftAdjustedConstraints(int columnSpan) {
        return new CellConstraints(isLeftToRight() ? getColumn() : (getColumn() + 1) - columnSpan, getRow(), columnSpan, cellConstraints().gridHeight);
    }
}
