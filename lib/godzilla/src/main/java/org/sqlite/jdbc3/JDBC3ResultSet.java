...
public abstract class JDBC3ResultSet extends CoreResultSet {
    ...
    public boolean next() throws SQLException {
        ...
        if (this.row == 0) {
            this.row++;
            return true;
        } else if (this.maxRows != 0 && this.row == this.maxRows) {
            return false;
        } else {
    ...
    public Date getDate(int col, Calendar cal) throws SQLException {
        ...
        switch (db.column_type(this.stmt.pointer, markCol(col))) {
            ...
            case 3:
                try {
                    return new Date(FastDateFormat.getInstance(getConnectionConfig().getDateStringFormat(), cal.getTimeZone()).parse(db.column_text(this.stmt.pointer, markCol(col))).getTime());
                } catch (Exception e) {
    ...
    public int getInt(int col) throws SQLException {
        return getDatabase().column_int(this.stmt.pointer, markCol(col));
    }
    ...
    public long getLong(int col) throws SQLException {
        return getDatabase().column_long(this.stmt.pointer, markCol(col));
    }
    ...
    public String getString(int col) throws SQLException {
        return getDatabase().column_text(this.stmt.pointer, markCol(col));
    }
    ...
    public Time getTime(int col, Calendar cal) throws SQLException {
        ...
        switch (db.column_type(this.stmt.pointer, markCol(col))) {
            ...
            case 3:
                try {
                    return new Time(FastDateFormat.getInstance(getConnectionConfig().getDateStringFormat(), cal.getTimeZone()).parse(db.column_text(this.stmt.pointer, markCol(col))).getTime());
                } catch (Exception e) {
                    SQLException error = new SQLException("Error parsing time");
                    error.initCause(e);
                    throw error;
    ...
    public Timestamp getTimestamp(int col, Calendar cal) throws SQLException {
        ...
        switch (db.column_type(this.stmt.pointer, markCol(col))) {
            ...
            case 3:
                try {
                    return new Timestamp(FastDateFormat.getInstance(getConnectionConfig().getDateStringFormat(), cal.getTimeZone()).parse(db.column_text(this.stmt.pointer, markCol(col))).getTime());
                } catch (Exception e) {
                    SQLException error = new SQLException("Error parsing time stamp");
                    error.initCause(e);
                    throw error;
    ...
    private Calendar julianDateToCalendar(Double jd, Calendar cal) {
        ...
        if (Z < 2299161) {
            ...
            int alpha = (int) ((((double) Z) - 1867216.25d) / 36524.25d);
            A = ((Z + 1) + alpha) - ((int) (((double) alpha) / 4.0d));
        }
        int B = A + 1524;
        int C = (int) ((((double) B) - 122.1d) / 365.25d);
        int D = (int) (365.25d * ((double) C));
        int E = (int) (((double) (B - D)) / 30.6001d);
        int mm = E - (((double) E) < 13.5d ? 1 : 13);
        int yyyy = C - (((double) mm) > 2.5d ? 4716 : 4715);
        double jjd = ((double) ((B - D) - ((int) (30.6001d * ((double) E))))) + F;
        int dd = (int) jjd;
        double hhd = jjd - ((double) dd);
        int hh = (int) (24.0d * hhd);
        double mnd = (24.0d * hhd) - ((double) hh);
        int mn = (int) (60.0d * mnd);
        double ssd = (60.0d * mnd) - ((double) mn);
        int ss = (int) (60.0d * ssd);
        cal.set(yyyy, mm - 1, dd, hh, mn, ss);
        cal.set(14, (int) (1000.0d * ((60.0d * ssd) - ((double) ss))));
        if (yyyy >= 1) {
            return cal;
        }
        cal.set(0, 0);
