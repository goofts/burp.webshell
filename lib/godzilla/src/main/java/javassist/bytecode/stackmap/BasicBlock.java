...
public class BasicBlock {
    ...
    public static class Mark implements Comparable<Mark> {
        ...
        /* access modifiers changed from: package-private */
        public void setJump(BasicBlock[] bb, int s, boolean always) {
            this.jump = bb;
            this.size = s;
    ...
    public static class Maker {
        ...
        public BasicBlock[] make(CodeIterator ci, int begin, int end, ExceptionTable et) throws BadBytecode {
            BasicBlock[] bb = makeBlocks(makeMarks(ci, begin, end, et));
            addCatchers(bb, et);
            return bb;
        }
        ...
        private Map<Integer, Mark> makeMarks(CodeIterator ci, int begin, int end, ExceptionTable et) throws BadBytecode {
            ...
            while (ci.hasNext() && (index = ci.next()) < end) {
                ...
                if ((153 <= op && op <= 166) || op == 198 || op == 199) {
                    makeMark(marks, index, makeArray(makeMark(marks, ci.s16bitAt(index + 1) + index).block, makeMark(marks, index + 3).block), 3, false);
                } else if (167 <= op && op <= 171) {
                    switch (op) {
                        case Opcode.GOTO:
                            makeGoto(marks, index, ci.s16bitAt(index + 1) + index, 3);
                            continue;
                        case Opcode.JSR:
                            makeJsr(marks, index, ci.s16bitAt(index + 1) + index, 3);
                            continue;
                        case Opcode.RET:
                        ...
                        case Opcode.TABLESWITCH:
                            int pos = (index & -4) + 4;
                            int ncases = (ci.s32bitAt(pos + 8) - ci.s32bitAt(pos + 4)) + 1;
                            BasicBlock[] to = makeArray(ncases + 1);
                            to[0] = makeMark(marks, ci.s32bitAt(pos) + index).block;
                            int p = pos + 12;
                            int n = p + (ncases * 4);
                            int k = 1;
                            while (p < n) {
                                k++;
                                to[k] = makeMark(marks, ci.s32bitAt(p) + index).block;
                                p += 4;
                            }
                        ...
                        case Opcode.LOOKUPSWITCH:
                            ...
                            BasicBlock[] to2 = makeArray(ncases2 + 1);
                            to2[0] = makeMark(marks, ci.s32bitAt(pos2) + index).block;
                            int p2 = pos2 + 8 + 4;
                            int n2 = ((ncases2 * 8) + p2) - 4;
                            int k2 = 1;
                            while (p2 < n2) {
                                k2++;
                                to2[k2] = makeMark(marks, ci.s32bitAt(p2) + index).block;
                                p2 += 8;
                            }
                            makeMark(marks, index, to2, n2 - index, true);
                            continue;
                    }
                } else if ((172 <= op && op <= 177) || op == 191) {
                    makeMark(marks, index, null, 1, true);
                } else if (op == 200) {
                    makeGoto(marks, index, ci.s32bitAt(index + 1) + index, 5);
                } else if (op == 201) {
                    makeJsr(marks, index, ci.s32bitAt(index + 1) + index, 5);
        ...
        private void makeGoto(Map<Integer, Mark> marks, int pos, int target, int size) {
            makeMark(marks, pos, makeArray(makeMark(marks, target).block), size, true);
        }

        /* access modifiers changed from: protected */
        public void makeJsr(Map<Integer, Mark> map, int pos, int target, int size) throws BadBytecode {
