...
public class MapMaker extends Tracer {
    ...
    public void make(TypedBlock[] blocks, byte[] code) throws BadBytecode {
        make(code, blocks[0]);
    ...
    private void traceException(byte[] code, BasicBlock.Catch handler) throws BadBytecode {
        while (handler != null) {
            ...
            if (tb.alreadySet()) {
                ...
                new MapMaker(this).make(code, tb);
            }
    ...
    private TypeData merge(TypeData src, TypeData target) throws BadBytecode {
        if (src != target && !(target instanceof TypeData.ClassName) && !(target instanceof TypeData.BasicType)) {
    ...
    private void recordStackMap0(TypedBlock target, int st, TypeData[] tStackTypes) throws BadBytecode {
        ...
        target.setStackMap(st, tStackTypes, recordTypeData(n, this.localsTypes, tLocalsTypes), tLocalsTypes);
    }
    ...
    public StackMapTable toStackMap(TypedBlock[] blocks) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            if (isTarget(bb, blocks[i - 1])) {
                bb.resetNumLocals();
                toStackMapBody(writer, bb, stackMapDiff(prev.numLocals, prev.localsTypes, bb.numLocals, bb.localsTypes), offsetDelta, prev);
    ...
    private void toStackMapBody(StackMapTable.Writer writer, TypedBlock bb, int diffL, int offsetDelta, TypedBlock prev) {
        ...
        if (stackTop == 0) {
            if (diffL == 0) {
                writer.sameFrame(offsetDelta);
                return;
            } else if (diffL < 0 && diffL >= -3) {
                writer.chopFrame(offsetDelta, -diffL);
                return;
            } else if (diffL > 0 && diffL <= 3) {
                int[] data = new int[diffL];
                writer.appendFrame(offsetDelta, fillStackMap(bb.numLocals - prev.numLocals, prev.numLocals, data, bb.localsTypes), data);
                return;
            }
        ...
        writer.fullFrame(offsetDelta, fillStackMap(bb.numLocals, 0, ldata, bb.localsTypes), ldata, stags, sdata);
    }
    ...
    public StackMap toStackMap2(ConstPool cp, TypedBlock[] blocks) {
        ...
        effective[0] = blocks[0].incoming > 0;
