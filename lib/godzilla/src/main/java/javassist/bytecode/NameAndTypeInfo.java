...
public class NameAndTypeInfo extends ConstInfo {
    ...
    public boolean equals(Object obj) {
        ...
        if (nti.memberName == this.memberName && nti.typeDescriptor == this.typeDescriptor) {
            return true;
        }
        return false;
    }
    ...
    public int copy(ConstPool src, ConstPool dest, Map<String, String> map) {
        return dest.addNameAndTypeInfo(dest.addUtf8Info(src.getUtf8Info(this.memberName)), dest.addUtf8Info(Descriptor.rename(src.getUtf8Info(this.typeDescriptor), map)));
    }
