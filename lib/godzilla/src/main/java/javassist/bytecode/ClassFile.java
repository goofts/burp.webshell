...
public final class ClassFile {
    ...
    public final void renameClass(String oldname, String newname) {
        if (!oldname.equals(newname)) {
            ...
            for (MethodInfo minfo : this.methods) {
                minfo.setDescriptor(Descriptor.rename(minfo.getDescriptor(), oldname2, newname2));
                AttributeInfo.renameClass(minfo.getAttributes(), oldname2, newname2);
            }
            for (FieldInfo finfo : this.fields) {
                finfo.setDescriptor(Descriptor.rename(finfo.getDescriptor(), oldname2, newname2));
    ...
    public final void renameClass(Map<String, String> classnames) {
        ...
        for (MethodInfo minfo : this.methods) {
            minfo.setDescriptor(Descriptor.rename(minfo.getDescriptor(), classnames));
            AttributeInfo.renameClass(minfo.getAttributes(), classnames);
        }
        for (FieldInfo finfo : this.fields) {
            finfo.setDescriptor(Descriptor.rename(finfo.getDescriptor(), classnames));
    ...
    public final void getRefClasses(Map<String, String> classnames) {
        ...
        for (FieldInfo finfo : this.fields) {
            Descriptor.rename(finfo.getDescriptor(), classnames);
    ...
    public void setMajorVersion(int major2) {
        this.major = major2;
    }
    ...
    public void setMinorVersion(int minor2) {
        this.minor = minor2;
    }
