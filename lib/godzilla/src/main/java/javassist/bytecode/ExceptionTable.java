...
public class ExceptionTable implements Cloneable {
    ...
    ExceptionTable(ConstPool cp, DataInputStream in) throws IOException {
        ...
        for (int i = 0; i < length; i++) {
            list.add(new ExceptionTableEntry(in.readUnsignedShort(), in.readUnsignedShort(), in.readUnsignedShort(), in.readUnsignedShort()));
        }
        this.entries = list;
    }
    ...
    public ExceptionTable copy(ConstPool newCp, Map<String, String> classnames) {
        ...
        for (ExceptionTableEntry e : this.entries) {
            et.add(e.startPc, e.endPc, e.handlerPc, srcCp.copy(e.catchType, newCp, classnames));
        }
        return et;
    }
