...
public class ClassFileWriter {
    ...
    public byte[] end(int accessFlags, int thisClass2, int superClass2, int[] interfaces, AttributeWriter aw) {
        this.constPool.end();
        this.output.writeShort(accessFlags);
        this.output.writeShort(thisClass2);
        this.output.writeShort(superClass2);
        if (interfaces == null) {
    ...
    public void end(DataOutputStream out, int accessFlags, int thisClass2, int superClass2, int[] interfaces, AttributeWriter aw) throws IOException {
        this.constPool.end();
        this.output.writeTo(out);
        out.writeShort(accessFlags);
        out.writeShort(thisClass2);
        out.writeShort(superClass2);
        if (interfaces == null) {
            out.writeShort(0);
        } else {
            int n = interfaces.length;
    ...
    public static final class FieldWriter {
        ...
        public void add(int accessFlags, String name, String descriptor, AttributeWriter aw) {
            add(accessFlags, this.constPool.addUtf8Info(name), this.constPool.addUtf8Info(descriptor), aw);
        }
    ...
    public static final class MethodWriter {
        ...
        public void addInvoke(int opcode, String targetClass, String methodName, String descriptor) {
            int method = this.constPool.addMethodrefInfo(this.constPool.addClassInfo(targetClass), this.constPool.addNameAndTypeInfo(methodName, descriptor));
            add(opcode);
            add16(method);
        }
        ...
        public void end(StackMapTable.Writer smap, AttributeWriter aw) {
            if (!this.isAbstract) {
                ...
                ClassFileWriter.writeAttribute(this.output, aw, smap == null ? 0 : 1);
                if (smap != null) {
