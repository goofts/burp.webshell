...
public class ClassFilePrinter {
    ...
    public static void print(ClassFile cf, PrintWriter out) {
        ...
        for (FieldInfo finfo : cf.getFields()) {
            out.println(Modifier.toString(AccessFlag.toModifier(finfo.getAccessFlags())) + " " + finfo.getName() + "\t" + finfo.getDescriptor());
            printAttributes(finfo.getAttributes(), out, 'f');
        }
        out.println();
        for (MethodInfo minfo : cf.getMethods()) {
            out.println(Modifier.toString(AccessFlag.toModifier(minfo.getAccessFlags())) + " " + minfo.getName() + "\t" + minfo.getDescriptor());
        ...
        if (list != null) {
            for (AttributeInfo ai : list) {
                if (ai instanceof CodeAttribute) {
                    ...
                    String sig = ((SignatureAttribute) ai).getSignature();
