...
public class LocalVariableAttribute extends AttributeInfo {
    ...
    public void renameClass(String oldname, String newname) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (index != 0) {
                ByteArray.write16bit(cp.addUtf8Info(renameEntry(cp.getUtf8Info(index), oldname, newname)), this.info, pos + 6);
            }
        }
    }
    ...
    public void renameClass(Map<String, String> classnames) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int index = ByteArray.readU16bit(this.info, pos + 6);
            if (index != 0) {
                ByteArray.write16bit(cp.addUtf8Info(renameEntry(cp.getUtf8Info(index), classnames)), this.info, pos + 6);
            }
        }
    }
    ...
    public AttributeInfo copy(ConstPool newCp, Map<String, String> classnames) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (type != 0) {
                type = newCp.addUtf8Info(Descriptor.rename(cp.getUtf8Info(type), classnames));
            }
