...
public class Annotation {
    ...
    public static MemberValue createMemberValue(ConstPool cp, CtClass type) throws NotFoundException {
        ...
        if (type.isArray()) {
            return new ArrayMemberValue(createMemberValue(cp, type.getComponentType()), cp);
        }
        if (type.isInterface()) {
            return new AnnotationMemberValue(new Annotation(cp, type), cp);
        }
    ...
    public int hashCode() {
        return (this.members == null ? 0 : this.members.hashCode()) + getTypeName().hashCode();
    }
