...
public class Javac {
    ...
    public Bytecode compileBody(CtBehavior method, String src) throws CompileError {
        ...
        boolean isVoid;
        try {
            recordParams(method.getParameterTypes(), Modifier.isStatic(method.getModifiers()));
            ...
            if (src == null) {
                ...
                Parser p = new Parser(new Lex(src));
                Stmnt s = p.parseStatement(new SymbolTable(this.stable));
                if (p.hasMore()) {
    ...
    private static void makeDefaultBody(Bytecode b, CtClass type) {
        ...
        if (type instanceof CtPrimitiveType) {
            op = ((CtPrimitiveType) type).getReturnOp();
            if (op == 175) {
                value = 14;
            } else if (op == 174) {
                value = 11;
    ...
    public void recordProceed(String target, final String method) throws CompileError {
        final ASTree texpr = new Parser(new Lex(target)).parseExpression(this.stable);
        this.gen.setProceedHandler(new ProceedHandler() {
            /* class javassist.compiler.Javac.AnonymousClass1 */
    ...
    public void recordStaticProceed(final String targetClass, final String method) throws CompileError {
        this.gen.setProceedHandler(new ProceedHandler() {
            /* class javassist.compiler.Javac.AnonymousClass2 */

            @Override // javassist.compiler.ProceedHandler
            public void doit(JvstCodeGen gen, Bytecode b, ASTList args) throws CompileError {
                gen.compileExpr(CallExpr.makeCall(Expr.make(35, new Symbol(targetClass), new Member(method)), args));
                gen.addNullIfVoid();
            }

            @Override // javassist.compiler.ProceedHandler
            public void setReturnType(JvstTypeChecker check, ASTList args) throws CompileError {
                CallExpr.makeCall(Expr.make(35, new Symbol(targetClass), new Member(method)), args).accept(check);
                check.addNullIfVoid();
            }
        }, proceedName);
    }
    ...
    public void recordSpecialProceed(String target, final String classname, final String methodname, final String descriptor, final int methodIndex) throws CompileError {
        final ASTree texpr = new Parser(new Lex(target)).parseExpression(this.stable);
        this.gen.setProceedHandler(new ProceedHandler() {
            /* class javassist.compiler.Javac.AnonymousClass3 */
    ...
    public void compileExpr(String src) throws CompileError {
        compileExpr(parseExpr(src, this.stable));
    }

    public static ASTree parseExpr(String src, SymbolTable st) throws CompileError {
        return new Parser(new Lex(src)).parseExpression(st);
    }
