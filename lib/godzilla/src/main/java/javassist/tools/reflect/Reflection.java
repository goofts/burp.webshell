...
public class Reflection implements Translator {
    ...
    private boolean isExcluded(String name) {
        if (name.startsWith("_m_") || name.equals(classobjectAccessor) || name.equals(metaobjectSetter) || name.equals(metaobjectGetter) || name.startsWith(readPrefix) || name.startsWith(writePrefix)) {
            return true;
        }
        return false;
    }
    ...
    public void onLoad(ClassPool pool, String classname) throws CannotCompileException, NotFoundException {
        pool.get(classname).instrument(this.converter);
    }
    ...
    public boolean makeReflective(CtClass clazz, CtClass metaobject, CtClass metaclass) throws CannotCompileException, CannotReflectException, NotFoundException {
        ...
        } else if (clazz.subclassOf(this.classPool.get(classMetaobjectClassName))) {
            throw new CannotReflectException("Cannot reflect a subclass of ClassMetaobject: " + clazz.getName());
        } else if (clazz.subclassOf(this.classPool.get(metaobjectClassName))) {
    ...
    private void processFields(CtClass clazz) throws CannotCompileException, NotFoundException {
        ...
        for (CtField f : fs) {
            ...
            if ((mod & 1) != 0 && (mod & 16) == 0) {
                ...
                clazz.addMethod(wmethod);
                CtMethod wmethod2 = CtNewMethod.wrapped(CtClass.voidType, writePrefix + name, new CtClass[]{this.classPool.get("java.lang.Object"), ftype}, null, this.trapWrite, CtMethod.ConstParameter.string(name), clazz);
