    ...
    public static long calculateDefault(CtClass clazz) throws CannotCompileException {
        try {
            ...
            Arrays.sort(fields, new Comparator<CtField>() {
                /* class javassist.SerialVersionUID.AnonymousClass1 */
            ...
            if (classFile.getStaticInitializer() != null) {
                out.writeUTF(MethodInfo.nameClinit);
                out.writeInt(8);
                out.writeUTF("()V");
            }
            CtConstructor[] constructors = clazz.getDeclaredConstructors();
            Arrays.sort(constructors, new Comparator<CtConstructor>() {
                /* class javassist.SerialVersionUID.AnonymousClass2 */
            ...
            Arrays.sort(methods, new Comparator<CtMethod>() {
                /* class javassist.SerialVersionUID.AnonymousClass3 */
            ...
            out.flush();
            byte[] digested = MessageDigest.getInstance("SHA").digest(bout.toByteArray());
            long hash = 0;
            for (int i2 = Math.min(digested.length, 8) - 1; i2 >= 0; i2--) {
                hash = (hash << 8) | ((long) (digested[i2] & 255));
            }
            return hash;
        } catch (IOException e) {
