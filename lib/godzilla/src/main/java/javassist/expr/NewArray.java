...
public class NewArray extends Expr {
    ...
    public CtClass getComponentType() throws NotFoundException {
        ...
        if (this.opcode == 189 || this.opcode == 197) {
            String desc = getConstPool().getClassInfo(this.iterator.u16bitAt(this.currentPos + 1));
            return Descriptor.toCtClass(Descriptor.toArrayComponent(desc, Descriptor.arrayDimension(desc)), this.thisClass.getClassPool());
        }
        throw new RuntimeException("bad opcode: " + this.opcode);
    }

    /* access modifiers changed from: package-private */
    public CtClass getPrimitiveType(int atype) {
        switch (atype) {
            case 4:
    ...
    public int getDimension() {
        ...
        if (this.opcode == 189 || this.opcode == 197) {
            int arrayDimension = Descriptor.arrayDimension(getConstPool().getClassInfo(this.iterator.u16bitAt(this.currentPos + 1)));
            if (this.opcode != 189) {
                i = 0;
            }
            return i + arrayDimension;
        }
        throw new RuntimeException("bad opcode: " + this.opcode);
    }
