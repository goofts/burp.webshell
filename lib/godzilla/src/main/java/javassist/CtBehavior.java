...
public abstract class CtBehavior extends CtMember {
    ...
    public boolean hasAnnotation(String typeName) {
        ...
        return CtClassType.hasAnnotationType(typeName, getDeclaringClass().getClassPool(), (AnnotationsAttribute) mi.getAttribute(AnnotationsAttribute.invisibleTag), (AnnotationsAttribute) mi.getAttribute(AnnotationsAttribute.visibleTag));
    }

    @Override // javassist.CtMember
    public Object getAnnotation(Class<?> clz) throws ClassNotFoundException {
        MethodInfo mi = getMethodInfo2();
        return CtClassType.getAnnotationType(clz, getDeclaringClass().getClassPool(), (AnnotationsAttribute) mi.getAttribute(AnnotationsAttribute.invisibleTag), (AnnotationsAttribute) mi.getAttribute(AnnotationsAttribute.visibleTag));
    }
    ...
    private Object[] getAnnotations(boolean ignoreNotFound) throws ClassNotFoundException {
        MethodInfo mi = getMethodInfo2();
        return CtClassType.toAnnotationType(ignoreNotFound, getDeclaringClass().getClassPool(), (AnnotationsAttribute) mi.getAttribute(AnnotationsAttribute.invisibleTag), (AnnotationsAttribute) mi.getAttribute(AnnotationsAttribute.visibleTag));
    }
    ...
    /* access modifiers changed from: package-private */
    public Object[][] getParameterAnnotations(boolean ignoreNotFound) throws ClassNotFoundException {
        MethodInfo mi = getMethodInfo2();
        return CtClassType.toAnnotationType(ignoreNotFound, getDeclaringClass().getClassPool(), (ParameterAnnotationsAttribute) mi.getAttribute(ParameterAnnotationsAttribute.invisibleTag), (ParameterAnnotationsAttribute) mi.getAttribute(ParameterAnnotationsAttribute.visibleTag), mi);
    }
    ...
    public void setBody(String src, String delegateObj, String delegateMethod) throws CannotCompileException {
        ...
        try {
            ...
            this.methodInfo.setCodeAttribute(jv.compileBody(this, src).toCodeAttribute());
    ...
    static void setBody0(CtClass srcClass, MethodInfo srcInfo, CtClass destClass, MethodInfo destInfo, ClassMap map) throws CannotCompileException {
        ...
        try {
            ...
            if (cattr != null) {
                destInfo.setCodeAttribute((CodeAttribute) cattr.copy(destInfo.getConstPool(), map2));
            }
    ...
    public void addLocalVariable(String name, CtClass type) throws CannotCompileException {
        ...
        ca.setMaxLocals(Descriptor.dataSize(desc) + maxLocals);
    }
    ...
    public void addParameter(CtClass type) throws CannotCompileException {
        ...
        try {
            addParameter2(Descriptor.paramSize(desc) + (Modifier.isStatic(getModifiers()) ? 0 : 1), type, desc);
            this.methodInfo.setDescriptor(desc2);
        } catch (BadBytecode e) {
            throw new CannotCompileException(e);
        }
    ...
    public void instrument(CodeConverter converter) throws CannotCompileException {
        this.declaringClass.checkModify();
        converter.doit(getDeclaringClass(), this.methodInfo, this.methodInfo.getConstPool());
    }
    ...
    private void insertBefore(String src, boolean rebuild) throws CannotCompileException {
        ...
        Javac jv = new Javac(cc);
        try {
            jv.recordParamNames(ca, jv.recordParams(getParameterTypes(), Modifier.isStatic(getModifiers())));
                ...
                if (c == 176 || c == 172 || c == 174 || c == 173 || c == 175 || c == 177) {
                    if (redundant) {
                        ...
                        if (noReturn) {
                            ...
                            jvc.recordParamNames(ca, jvc.recordParams(getParameterTypes(), Modifier.isStatic(getModifiers())));
                        ...
                        iterator.append(bcode.getExceptionTable(), iterator.append(bcode.get()));
                        insertGoto(iterator, iterator.getCodeLength() - adviceLen2, pos);
    ...
    private void insertGoto(CodeIterator iterator, int subr, int pos) throws BadBytecode {
        ...
        if (wide) {
            iterator.writeByte(Opcode.GOTO_W, pos2);
            iterator.write32bit(offset, pos2 + 1);
        } else if (offset <= 32767) {
            iterator.writeByte(Opcode.GOTO, pos2);
    ...
    public void addCatch(String src, CtClass exceptionType, String exceptionName) throws CannotCompileException {
        ...
        try {
            ...
            b.addAstore(jv.recordVariable(exceptionType, exceptionName));
            jv.compileStmnt(src);
            int stack = b.getMaxStack();
            int locals = b.getMaxLocals();
            if (stack > ca.getMaxStack()) {
