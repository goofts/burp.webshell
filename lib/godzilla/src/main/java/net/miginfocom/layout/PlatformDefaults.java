...
public final class PlatformDefaults {
    ...
    public static float getMinimumButtonWidthIncludingPadding(float refValue, ContainerWrapper parent, ComponentWrapper comp) {
        ...
        if (comp == null || getMinimumButtonPadding() == null) {
            return (float) buttonMinWidth;
        }
        return (float) Math.max(comp.getMinimumWidth(comp.getWidth()) + (getMinimumButtonPadding().getPixels(refValue, parent, comp) * 2), buttonMinWidth);
    }
    ...
    static int convertToPixels(float value, String unit, boolean isHor, float ref, ContainerWrapper parent, ComponentWrapper comp) {
        ...
        return uv != null ? Math.round(((float) uv.getPixels(ref, parent, comp)) * value) : UnitConverter.UNABLE;
    }
    ...
    static String getTagForChar(char c) {
        switch (c) {
            ...
            case 'e':
                return "help2";
            case 'h':
                return FlatClientProperties.BUTTON_TYPE_HELP;
            case 'i':
                return "finish";
            case Opcode.IDIV:
                return "left";
