...
public final class LayoutUtil {
    ...
    static synchronized void setDelegate(java.lang.Class<?> r3, java.beans.PersistenceDelegate r4) {
        /*
            java.lang.Class<net.miginfocom.layout.LayoutUtil> r1 = net.miginfocom.layout.LayoutUtil.class
            monitor-enter(r1)
            r0 = 3
            java.beans.BeanInfo r0 = java.beans.Introspector.getBeanInfo(r3, r0)     // Catch:{ Exception -> 0x0016, all -> 0x0013 }
            java.beans.BeanDescriptor r0 = r0.getBeanDescriptor()     // Catch:{ Exception -> 0x0016, all -> 0x0013 }
            java.lang.String r2 = "persistenceDelegate"
            r0.setValue(r2, r4)     // Catch:{ Exception -> 0x0016, all -> 0x0013 }
        L_0x0011:
            monitor-exit(r1)
            return
        L_0x0013:
        ...
        L_0x0016:
            ...
            goto L_0x0011
        */
        throw new UnsupportedOperationException("Method not decompiled: net.miginfocom.layout.LayoutUtil.setDelegate(java.lang.Class, java.beans.PersistenceDelegate):void");
    }

    static String getCCString(Object con) {
        if (CR_MAP != null) {
            return CR_MAP.get(con);
        }
        return null;
    }
    ...
    static int[] calculateSerial(int[][] sizes, ResizeConstraint[] resConstr, Float[] defPushWeights, int startSizeType, int bounds) {
        ...
        for (int i = 0; i < sizes.length; i++) {
            if (sizes[i] != null) {
                float len = sizes[i][startSizeType] != NOT_SET ? (float) sizes[i][startSizeType] : 0.0f;
                int newSizeBounded2 = getBrokenBoundary(len, sizes[i][0], sizes[i][2]);
                if (newSizeBounded2 != NOT_SET) {
                    len = (float) newSizeBounded2;
                }
                usedLength += len;
                lengths[i] = len;
            }
        ...
        if (useLengthI != bounds && resConstr != null) {
            ...
            for (int i2 = 0; i2 < sizes.length; i2++) {
                ResizeConstraint resC2 = (ResizeConstraint) getIndexSafe(resConstr, i2);
                if (resC2 != null) {
                    prioList.add(Integer.valueOf(isGrow ? resC2.growPrio : resC2.shrinkPrio));
                }
            }
            Integer[] prioIntegers = (Integer[]) prioList.toArray(new Integer[prioList.size()]);
            int force = 0;
            while (true) {
                if (force > ((!isGrow || defPushWeights == null) ? 0 : 1)) {
                    break;
                }
                for (int pr = prioIntegers.length - 1; pr >= 0; pr--) {
                    ...
                    while (i3 < sizes.length) {
                        if (!(sizes[i3] == null || (resC = (ResizeConstraint) getIndexSafe(resConstr, i3)) == null)) {
                            if (curPrio == (isGrow ? resC.growPrio : resC.shrinkPrio)) {
                                if (isGrow) {
                                    resizeWeight[i3] = (force == 0 || resC.grow != null) ? resC.grow : defPushWeights[i3 < defPushWeights.length ? i3 : defPushWeights.length - 1];
                                } else {
                                    resizeWeight[i3] = resC.shrink;
                                }
                                if (resizeWeight[i3] != null) {
                                    totWeight += resizeWeight[i3].floatValue();
                                }
                            }
                        }
                        i3++;
                    ...
                    if (totWeight > 0.0f) {
                        do {
                            ...
                            for (int i4 = 0; i4 < sizes.length && totWeight > 1.0E-4f; i4++) {
                                Float weight = resizeWeight[i4];
                                if (weight != null) {
                                    float sizeDelta = (weight.floatValue() * toChange) / totWeight;
                                    float newSize = lengths[i4] + sizeDelta;
                                    if (!(sizes[i4] == null || (newSizeBounded = getBrokenBoundary(newSize, sizes[i4][0], sizes[i4][2])) == NOT_SET)) {
                                        resizeWeight[i4] = null;
                                        hit = true;
                                        changedWeight += weight.floatValue();
                                        newSize = (float) newSizeBounded;
                                        sizeDelta = newSize - lengths[i4];
                                    }
                                    lengths[i4] = newSize;
    ...
    private static int getBrokenBoundary(float sz, int lower, int upper) {
        ...
        if (upper == NOT_SET || sz <= ((float) upper)) {
            return NOT_SET;
        }
        return upper;
    }
    ...
    static BoundSize derive(BoundSize bs, UnitValue min, UnitValue pref, UnitValue max) {
        ...
        return new BoundSize(min2, preferred, max2, bs.getGapPush(), null);
    }
    ...
    static UnitValue getInsets(LC lc, int side, boolean getDefault) {
        ...
        if (i == null || i[side] == null) {
            return getDefault ? PlatformDefaults.getPanelInsets(side) : UnitValue.ZERO;
        }
        return i[side];
    }
    ...
    public static synchronized void writeAsXML(ObjectOutput out, Object o) throws IOException {
        synchronized (LayoutUtil.class) {
            ...
            writeXMLObject(writeOutputStream, o, new ExceptionListener() {
                /* class net.miginfocom.layout.LayoutUtil.AnonymousClass1 */
